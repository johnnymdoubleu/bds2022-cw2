...
kable(head(study), caption = "Linear regression on Pheno vs SNP") %>%
  kable_styling(latex_options = "hold_position")
```
```{r}
...
confidence.int.95 <- round(exp(strong.coef + 1.96 * strong.coef*c(-1,1)), 3)
confidence.int.99 <- round(exp(strong.coef + 2.576 * strong.coef*c(-1,1)),3)
