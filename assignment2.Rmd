...
table1 <-data.table(model = c("Lasso.min", "Lasso.1se", "Ridge.min", "Ridge.1se"),
                    Lambda = c(signif(lambdamin.lasso,3), signif(lambda1se.lasso,3), signif(lambdamin.ridge,3), signif(lambda1se.ridge,3)), 
                    ModelSize = c(ms.lasso.min, ms.lasso.1se, ms.ridge.min, ms.ridge.1se),
                    AUC = c(AUC.lambdamin.lasso, AUC.lambda1se.lasso, AUC.lambdamin.ridge, AUC.lambda1se.ridge))
table1 
```
...
full_model <- glm(diagnosis~., data = train.breast)
Null_model <- glm(diagnosis~1, data = train.breast)

model.B<- stepAIC(full_model, direction = "back")
model.S<- stepAIC(Null_model, scope = list(upper = full_model), direction ="forward")
```
...
kable(list(B.coef[B.order], S.coef[S.order]),col.names = "Coefficients")
```
...
lasso.auc <- roc(test.breast$diagnosis, lasso.pred, plot =TRUE, col = "red")$auc
